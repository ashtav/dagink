...
text(text, {color, size: 15, bold: false, TextAlign align: TextAlign.left, spacing: 0, double height: 1.3, font: 'Nunito', TextOverflow overflow}){
  return Text(
...
class Wh {
  ...
  static noData({image: 'no-doc.png', message: '', Function onTap}){
    return Center(
      child: ZoomIn(
        child: Column(
          ...
          children: [
            ...
            text(message, align: TextAlign.center, color: Colors.black54)
  ...
  static options(context, {String label, @required List options, List values, List icons, List hide, double radius: 5, Function then, bool backOnSelected: false, List danger}){
                                  ...
                                  child: Row(
                                    ...
                                    children: <Widget>[
                                      Container(
                                        ...
                                        child: text(options[i], align: TextAlign.left, overflow: TextOverflow.ellipsis, color: danger == null ? Colors.black54 : danger.indexOf(i) > -1 ? Colors.red : Colors.black54)
                                      ),
                                      
                                      icons != null ? Icon(icons[i], size: 18, color: danger == null ? Colors.black54 : danger.indexOf(i) > -1 ? Colors.red : Colors.black54) : SizedBox.shrink()
                                    ],
                                  )
                                ),
                              ),
  ...
  static alert(context, {dismiss: true, Color color, String title, message: '', Icon icon}){
    showDialog(
      context: context,
      barrierDismissible: dismiss,
      builder: (BuildContext context) {
  ...
  static appBar(context, {title = '', Color color, elevation = 1, back: true, spacing: 15, List<Widget> actions, bool autoLeading: false, bool center: false, Widget leading}){
    return new AppBar(
      iconTheme: IconThemeData(size: 3), actionsIconTheme: IconThemeData(size: 3),
      centerTitle: center,
      backgroundColor: color == null ? Colors.white : color,
      automaticallyImplyLeading: autoLeading,
      titleSpacing: back ? 0 : 15,
      elevation: elevation.toDouble(),
      leading: leading != null ? leading : back ? IconButton( onPressed: (){ Navigator.pop(context); }, icon: Icon(Ln.angleLeft()), color: Colors.black54, ) : null,
                  ...
                  child: Row(
                    ...
                    children: <Widget>[
                      ...
                      Container(
                        color: Colors.transparent,
                        padding: EdgeInsets.only(left: 10),
                        child: text(widget.radioLabels == null ? ucwords(widget.values[i].toString().replaceAll('_', ' ')) : widget.radioLabels[i])
                      )
...
class SelectGroup extends StatefulWidget {
  SelectGroup({this.label, @required this.options, this.labels, this.labelsUppercase: false, @required this.controller, });
...
class InputNumber extends StatefulWidget {
  InputNumber({this.label, this.hint, this.controller, this.action, this.enabled: true, this.submit, this.change, this.node, this.length});
...
class _ListSkeletonState extends State<ListSkeleton> {
  @override
  Widget build(BuildContext context) {
    return widget.type == null ? SingleChildScrollView(
                ...
                Expanded(
                  child: Column(
                    children: <Widget>[
                      ...
                      selected: widget.selected, change: (res){
                        selected = res;
                      })
                    ]
                  )
